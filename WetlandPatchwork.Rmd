...
values(RE.r) = exp((-model7$summary.random$Region[,2]/12) - 1) * 100
...
M7RE = levelplot(RE.r, margin = FALSE,
...
###Model Run Times
```{r}
Time.df = as.data.frame(rbind(D100Time, D30Time, D70Time, D200Time, D300Time))
colnames(Time.df) = "Time"

Time.df$Cnts = c(D100cnt, D30cnt, D70cnt, D200cnt, D300cnt)

TimeS = arrange(Time.df, Cnts)

ggplot(TimeS, aes(Cnts, Time)) + 
       geom_point() +
...
print.data.frame(model5$summary.fixed)
...
Col_M5.df = Wetlands.spdf@data %>%
            select(V1, AWC, OM, SAVI, TPI) 

M5_col = colldiag(Col_M5.df)
M5_col
detach("package:perturb", unload=TRUE)
```
...
```{r}
RE.r = Blank.r
Wetlands.spdf$SREm5 = model5$summary.random$Region[,2]
values(RE.r) = (exp(model5$summary.random$Region$mean/10) - 1) * 100

REffect = round(Wetlands.spdf$SREm5, 4)
Var = rep("RE", length(REffect))
RE.df = data.frame(Var, REffect)
RE.df$REffect = as.numeric(RE.df$REffect)
...
M5RE = levelplot(RE.r, margin = FALSE,
          sub = "                Spatial Random Effects (model5)                \n(Above/Below Domain Mean)",
          xlab = NULL, ylab = NULL, 
          col.regions = cr, at = rng,
          colorkey = list(at = rng, labels = rngL, col = cr),
          par.settings = list(fontsize = list(text = 13)))  
...
```{r}
M5NW = Blank.r
values(M5NW) = Wetlands.spdf@data$M5Prob

rng = seq(0, 1, 0.10)
rngL = paste(rng*100, '%', sep = "")
cr = brewer.pal(10, "BrBG")
M5NW.r = levelplot(M5NW, margin = FALSE, 
          sub = "Probabilty of Wetland Presence             ",
          xlab = NULL, ylab = NULL, 
          col.regions = cr, at = rng,
          colorkey = list(at = rng, labels = rngL, col = cr),
...
NW_Mwcnt.df = filter(Wetlands.spdf@data, M5Prob >= 0.5)
NW_M5cnt = length(NW_Mwcnt.df$M5Prob)
```
...
```{r}
rm(list=setdiff(ls(), c("FIX", "model7", "model7_30", "model7_70", "model7_200", "M5_NW",
                        "model7_300", "Brier", "M7_NW2", "M5NW", "SDAquery", "NW_M5cnt")))
```
...
#M10.df = as.data.frame(model10$summary.fixed)
#colnames(M10.df) = c("Mean", "sd", "Quant0.025", "Quant0.5", "Quant0.975", "mode", "kld")
#M10b.df = M10.df %>%
#            select(Mean, Quant0.025, Quant0.5, Quant0.975)

#Mod10Table = xtable(M10b.df, 
#                   caption = "Summary of Fixed Values (model10)", 
#                  digits=c(0,4,4,4,4))
#print(Mod10Table, 
#     include.rownames = TRUE, 
 #     floating = TRUE )
...
Col_M10.df = Wetlands.spdf@data %>%
            select(V1, V4, MCAV2, MCAV3, AWC, SAVI, TPI, CTI) 

M10_col = colldiag(Col_M10.df)
M10_col
...
values(RE.r) = (exp(model6$summary.random$Region$mean/10) - 1) * 100
...
rngL = paste(rng*100, '%', sep = "")
cr = brewer.pal(10, "BrBG")
M5NW.r = levelplot(MSW, margin = FALSE, 
          sub = "Probabilty of Wetland Presence             ",
          xlab = NULL, ylab = NULL, 
          col.regions = cr, at = rng,
          colorkey = list(at = rng, labels = rngL, col = cr),
...
Wet_ChangeNW = sum(nwi2.NW, M5NW)
stackNW = stack(M5NW, nwi.NW, Wet_ChangeNW)
...
WetDiffnw = ((NW_M5cnt - NW_NWIcnt)/NW_NWIcnt)*100
...
```{r}
rm(list=setdiff(ls(), c("model7", "model7_30", "model7_70", "model7_200", "M5_NW",
                        "model7_300", "Brier", "M7_NW2", "M7_SW", "M16_SW")))
```
